...
export async function GET(request: Request) {
  ...
  if (isNaN(parsedUserId) || isNaN(parsedAmount) || parsedAmount <= 0) {
    console.error('Lootably Postback: Invalid user ID or amount', { userId, amount });
    return NextResponse.json({ message: 'Invalid user ID or amount' }, { status: 400 });
  }
    ...
    await getPrisma().$transaction(async (tx) => {
      ...
      if (!userPoints) {
        ...
        userPoints = await tx.userPoints.create({
          data: { userId: parsedUserId, pointsBalance: parsedAmount },
        });
      } else {
