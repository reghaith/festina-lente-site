...
export async function GET(request: Request) {
  ...
  if (isNaN(parsedReward) || parsedReward <= 0) {
    console.error('CPX Postback: Invalid reward amount', { reward });
    return NextResponse.json({ message: 'Invalid reward amount' }, { status: 400 });
  }
      ...
      let userPoints = await tx.userPoints.findUnique({
       ...
       if (!userPoints) {
        ...
        userPoints = await tx.userPoints.create({
          data: { userId: userId, pointsBalance: parsedReward },
        });
      } else {
